...
export class ServiceService {
  ...
  getAllLectures(courseId) {
    return this.http.get(this.url + API_ENDPOINTS.listOfLectures + courseId, this.requestHeaders);
    // return this.http.get('./assets/jsons/all-lectures.json');
  }

  getSelectedLectureDays(data) {
    console.log("objjjjjj = ", data);
    let url = `${this.url}${API_ENDPOINTS.getSelectedDays}${data.lId}/${data.cId}`
    return this.http.get(url, this.requestHeaders);
    // return this.http.get('./assets/jsons/selected-days.json');
  }

  deleteLecture(params) {
    return this.http.post(API_ENDPOINTS.removeLecture, params, this.requestHeaders);
  }
